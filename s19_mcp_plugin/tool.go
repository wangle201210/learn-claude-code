...
func init() {
	...
	// s18 新增：Lead 端的 3 个 worktree 工具。
	tools = append(tools, tool{
		info: &schema.ToolInfo{
			Name: "create_worktree",
			Desc: "Create an isolated git worktree at .worktrees/{name} on branch wt/{name}. Optionally bind to a task.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
				"name":    {Type: schema.String, Desc: "Worktree name (letters/digits/dots/underscores/dashes, 1-64).", Required: true},
				"task_id": {Type: schema.String, Desc: "Optional task ID to bind."},
			}),
		},
		handler: runCreateWorktree,
	})
	tools = append(tools, tool{
		info: &schema.ToolInfo{
			Name: "remove_worktree",
			Desc: "Remove a worktree and its branch. Refuses if there are uncommitted changes unless discard_changes=true.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
				"name":            {Type: schema.String, Desc: "Worktree name.", Required: true},
				"discard_changes": {Type: schema.Boolean, Desc: "true to force remove even with uncommitted changes."},
			}),
		},
		handler: runRemoveWorktree,
	})
	tools = append(tools, tool{
		info: &schema.ToolInfo{
			Name: "keep_worktree",
			Desc: "Mark a worktree as kept (preserved for manual review). Branch remains intact.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
				"name": {Type: schema.String, Desc: "Worktree name.", Required: true},
			}),
		},
		handler: runKeepWorktree,
	})

	// s19 新增：connect_mcp 把一个 MCP server（mock）连上，工具进入动态池。
	tools = append(tools, tool{
		info: &schema.ToolInfo{
			Name: "connect_mcp",
			Desc: "Connect to a mock MCP server (e.g. 'docs' or 'deploy'). Discovered tools become mcp__{server}__{tool}.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
				"name": {Type: schema.String, Desc: "MCP server name to connect (mock: docs, deploy).", Required: true},
			}),
		},
		handler: runConnectMCP,
	})
}
