...
func agentLoop(ctx context.Context, agent model.ToolCallingChatModel, messages []*schema.Message) ([]*schema.Message, error) {
	...
	for {
		// s19: 每轮重组工具 dispatch（含 builtin + 已连接 MCP server 的工具），
		// 这样 connect_mcp 之后新工具立刻可被调度。
		dispatch := assembleHandlers()
		...
		// s15+s16: 顶部消费 lead 邮箱；consumeLeadInbox 同时把 *_response
		// 类型的协议消息路由到 matchResponse（更新 pendingRequests 状态）。
		if inbox := consumeLeadInbox(true); len(inbox) > 0 {
