...
func teammateToolInfos() []*schema.ToolInfo {
	return []*schema.ToolInfo{
		...
		// s17 新增：teammate 可以扫任务板、认领、完成（自治协作）。
		{
			Name:        "list_tasks",
			Desc:        "List all tasks on the board.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{}),
		},
		{
			Name: "claim_task",
			Desc: "Claim a pending task (with deps completed). Sets owner to yourself.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
		...
		{
			Name: "complete_task",
			Desc: "Mark an in-progress task as completed; reports newly unblocked downstream tasks.",
			ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
				"task_id": {Type: schema.String, Desc: "Task identifier.", Required: true},
			}),
		},
...
func spawnTeammateThread(name, role, prompt string) string {
	...
	go func() {
		...
		dispatch := map[string]toolHandler{
			...
			// s17: teammate 自己也能看任务板、认领、完成（owner 是 teammate name）。
			"list_tasks": runListTasks,
			"claim_task": func(ctx context.Context, args string) string {
				var a struct {
					TaskID string `json:"task_id"`
				}
				_ = json.Unmarshal([]byte(args), &a)
				return claimTask(a.TaskID, name)
			},
		...
		// appendUser 让 idlePoll 把 auto-claim 等通知写回这个 goroutine 的 messages。
		appendUser := func(s string) { messages = append(messages, schema.UserMessage(s)) }

		// s17 外层 WORK→IDLE→WORK 生命周期。
		const workRoundsPerPhase = 10
	autonomous:
		for !shutdownReq {
			// ── WORK phase ────────────────────────────────────
			for workRound := 0; workRound < workRoundsPerPhase && !shutdownReq; workRound++ {
				if stop, _ := dispatchInbox(busReadInbox(name)); stop {
					shutdownReq = true
					break autonomous
				}
			...
			// ── IDLE phase（s17 新）──────────────────────────
			switch idlePoll(name, dispatchInbox, appendUser) {
			case "shutdown":
				shutdownReq = true
			case "timeout":
				break autonomous
