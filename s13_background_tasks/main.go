...
func main() {
	...
	fmt.Println("s13: Background Tasks — 慢操作走 goroutine，结果以通知注入")
	fmt.Println("输入问题，回车发送。输入 q 退出。")
	fmt.Println()

	messages := []*schema.Message{schema.SystemMessage(getSystemPrompt(buildContext()))}

	for {
		query, ok := readLine("\033[36ms13 >> \033[0m")
		if !ok {
			break
		}
